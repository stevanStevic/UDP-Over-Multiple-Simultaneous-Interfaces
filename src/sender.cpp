        ...
        */
        ...
        while ((result = pcap_next_ex(device_handle_eth, &packet_header, &packet_data)) >= 0) {
            if(result == 1) {
                ...
                std::cout << "eth ack: " << ack_f->ack_num  << std::endl;
...
void wlanThreadFunction(pcap_if_t* device, Segmenter* segmenter) {
    ...
    // Compile the filter
    if (pcap_compile(device_handle_wlan, &fcode, filter_exp, 1, netmask) < 0)
    {
        printf("\n Unable to compile the packet filter. Check the syntax.\n");
        return;
    }
...
/*
        ...
        */
        ...
        fill_data_frame(&frame_to_send, src_mac_wlan, dest_mac_wlan, pd.data, pd.data_num, segmenter->getNumOfPcks(), pd.data_size, src_ip_wlan, dest_ip_wlan);
        pcap_sendpacket(device_handle_wlan, (const unsigned char*)&frame_to_send, sizeof(frame));

        while ((result = pcap_next_ex(device_handle_wlan, &packet_header, &packet_data)) >= 0) {
            if(result == 1) {
                ack_frame* ack_f = (ack_frame*)packet_data;
                std::cout << "wlan ack: " << ack_f->ack_num  << std::endl;
                if(ack_f->ack_num == pd.data_num) {
                    delete []pd.data;
                    break;
                }
                ...
                if(++tryCountNotRecieved == 5) {
                    segmenter->putPartBack(pd);
                    std::cout << "Wlan device not available any more" << std::endl;
                    return;
                }
                else {
                    std::cout << "Wlan timeout expired. Sending packet " << pd.data_num << " again. Try count: " << tryCountNotRecieved << std::endl;
                }
            }
